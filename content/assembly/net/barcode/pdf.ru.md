...
############################# Head ############################
head_title: "Генерация штрих-кодов в документах PDF с помощью C#"
...
############################# Header ############################
title: "Добавьте штрих-коды в документы PDF с помощью нашего API .NET" 
description: "GroupDocs.Assembly for .NET полностью поддерживает создание и встраивание штрих-кодов в документы PDF динамически."
...
steps:
    enable: true
    title: "Шаги для добавления сгенерированного штрих-кода в документ PDF"
    content: |
      [GroupDocs.Assembly](/assembly/net/) упрощает вставку штрих-кодов в шаблоны в таких форматах, как PDF. Поддерживает более 60 типов штрих-кодов, включая одноразмерные и двумерные форматы.
      
      1. Создайте шаблон с конкретными тегами для размещения штрих-кода (учтите: PDF-шаблоны несовместимы).
      ...
      4. Сохраните финальный документ с вставленным штрих-кодом в формате PDF.
    ...
    code:
      ...
      content: |
        ...
        // Поддержка PDF-шаблонов в настоящее время недоступна.
...
more_features:
  ...
  code_samples_ext:
    ...
    - title: "Как динамически генерировать штрих-код"
      content: |
        Этот пример демонстрирует встраивание динамически сгенерированного штрих-кода в документ PDF.
      code:
        title: "C#"
        content: |
          ```csharp {style=abap}
