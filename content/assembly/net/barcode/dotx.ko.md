...
############################# Header ############################
title: ".NET을 통해 Word BARCODE 문서에서 바코드 이미지 생성 및 사용"
description: "GroupDocs.Assembly .NET API 프로그래머를 사용하면 C#, ASP.NET 및 기타 .NET 앱 내부의 Word BARCODE 문서에서 바코드 이미지를 동적으로 생성 및 관리할 수 있습니다."
...
steps:
    enable: true
    block:
    - title_left: "BARCODE 문서에 바코드를 만드는 방법"
      content_left: |
       다음 .NET 코드 예제는 사용자가 몇 줄의 코드로 자신의 Microsoft Word BARCODE 문서 내에 바코드 이미지를 얼마나 쉽게 동적으로 생성하고 추가할 수 있는지 보여줍니다.  

      title_right: ".NET을 통해 BARCODE 파일의 바코드 이미지 사용"
      content_right: |
