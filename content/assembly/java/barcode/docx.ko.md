...
############################# Head ############################
head_title: "Java로 DOCX 파일에 바코드를 삽입하기"
head_description: "GroupDocs.Assembly for Java API를 사용하면 문서와 이메일에 실시간으로 바코드 이미지를 생성하고 삽입하는 것이 간편합니다."

############################# Header ############################
title: "우리의 Java API로 DOCX 파일을 위한 바코드 생성하기" 
description: "GroupDocs.Assembly for Java은 DOCX 파일에 바코드를 동적으로 생성, 사용자 정의 및 삽입하는 포괄적인 도구를 제공합니다."
...
steps:
    enable: true
    title: "DOCX 문서에 바코드를 삽입하는 방법"
    content: |
      [GroupDocs.Assembly](/assembly/java/)을 사용하면 DOCX 템플릿과 같은 인기 있는 형식에 바코드를 삽입할 수 있습니다. 1D 및 2D 바코드를 포함하여 60개 이상의 형식을 지원합니다.
      
      1. DOCX 템플릿을 바코드 마커와 함께 설정합니다.
...
more_features:
  ...
  code_samples_ext:
    ...
    - title: "동적으로 바코드 생성하기"
      content: |
        이 예제에서는 DOCX 문서에 동적으로 바코드를 생성하고 추가하는 방법을 보여줍니다.
      code:
        title: "Java"
        content: |
          ```java {style=abap}
