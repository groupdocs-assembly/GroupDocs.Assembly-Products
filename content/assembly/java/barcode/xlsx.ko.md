...
draft: false
path: "ko/assembly/java/barcode/xlsx/"
...
steps:
    enable: true
    block:
    - title_left: "자바를 통해 XLSX 스프레드시트에 바코드 생성"
      content_left: |
       GroupDocs.Assembly Java는 XLSX 스프레드시트 내에서 바코드 생성 및 관리를 완벽하게 지원합니다. 다음 Java 코드는 Microsoft Excel 스프레드시트 문서 내에 바코드 이미지를 만들고 삽입하는 방법을 보여줍니다. 

      title_right: "XLSX 파일에 바코드 이미지를 추가하는 방법"
      content_right: |
