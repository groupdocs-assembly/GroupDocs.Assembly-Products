...
############################# Header ############################
title: "JavaAPIを介してWord BARCODE ドキュメントでバーコード画像を生成する"
description: "GroupDocs.Assembly java APIを使用すると、ソフトウェア開発者は、Javaアプリケーション内のWord BARCODE ドキュメント内でバーコードイメージを動的に作成および変更できます。」"
...
steps:
    enable: true
    block:
    - title_left: "BARCODE ドキュメントでのバーコード画像の生成"
      content_left: |
       次のjavaコード例は、Microsoft Word BARCODE ドキュメント内でのバーコードイメージの動的な作成と挿入を示しています。 開発者は、わずか数行のJavaコードを使用してタスクを実行できます。

      title_right: "Javaを介して BARCODE ファイルにバーコードを追加する"
      content_right: |
