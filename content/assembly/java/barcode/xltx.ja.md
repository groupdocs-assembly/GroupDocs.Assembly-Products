...
steps:
    enable: true
    block:
    - title_left: "Javaを介してXLTXスプレッドシートでバーコードを作成する"
      content_left: |
       GroupDocs.Assembly Javaは、XLTXスプレッドシート内でバーコードを作成および管理するための完全なサポートを提供します。次のJavaコードは、Microsoft Excelスプレッドシートドキュメント内にバーコード画像を作成して挿入する方法を示しています。 
